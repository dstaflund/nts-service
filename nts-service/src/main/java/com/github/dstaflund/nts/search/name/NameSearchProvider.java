    ...
    static List<NtsMap> findMapsByName(NameSearchParams ctx) {
        return QueryExecuter.executeQuery((Session session) ->
            session
                ...
                .setParameter(PARAM_NAME, getSearchValue(ctx.getName()))
                .setParameter(PARAM_SNIPPET, ctx.getSnippet())
                .setParameter(PARAM_PARENT, getSearchValue(ctx.getParent()))
        );
    }

    private static String getSearchValue(String value){
